...
2. `profile.md` — the user's targeting, out-of-scope filters, target families
   & bases, résumé register/structure/style, and voice (incl. letterhead, length,
   forbidden phrases). `SPEC.md` — Targets & bases (mechanism), Cover letters.
...
5. `SPEC.md` — Cover letters: the shape. `profile.md` — voice, letterhead, length, forbidden phrases.
...
If the JD is **out of scope** per profile.md Targeting (pure staff-SWE, dated QA-only with no
...
- Honor profile.md Voice: no keyword soup, no fog verbs, no self-assessment
  flourishes, no "X in; Y out", no em-dashes. Honor profile.md Résumé (the engine
  enforces style; you just supply clean markdown).

## Step 3 — write `<folder>/cover-letter.md`

- Follow SPEC Cover letters shape: ~180–300 words, signed `W.S. Gong`. Format:
...
- Open with W.S.'s own framing of why the role fits — name the role and company,
  ...
  **disposition close** (settled temperament stated as plain fact). **Never** a
  plan-close ("the first thing I'd do is…") — that is a banned tell (SPEC Cover letters).
- Voice from `voice/` (see profile.md Voice). **Never** quote the company's marketing/news
  back at them; no "what excites me about"; no forbidden phrases; no em-dashes;
  body ≤ the profile.md Length hard max (500). No `[NEEDS SOURCE]` markers — if you
